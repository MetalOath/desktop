...
| Package (macOS .dmg) | `cd engine && ./mach package` |
| Lint | `npm run lint` |
...
After a successful `npm run build` (or `cd engine && ./mach build`), create a distributable `.dmg`:

```bash
cd engine && ./mach package
# Output: obj-aarch64-apple-darwin/dist/zen-<version>.en-US.mac.dmg
```

The `update_en_US_packs.py` step is mandatory before the first build — skipping it produces a browser with blank context menus and missing UI strings.

## Building for Windows

Run these steps on a Windows machine:

**Prerequisites**

```powershell
winget install Git.Git Mozilla.MozillaBuild Node.js
```

Then install Rust via [rustup.rs](https://rustup.rs) and Python 3.11 via [python.org](https://python.org).

**Build**

```powershell
git clone https://github.com/MetalOath/desktop.git zen-browser
cd zen-browser
git checkout feat/webextension-sidebar-order-api   # or whichever branch
npm ci
npm run download
npm run import
npm run bootstrap
...
.\mach build
.\mach package
# Output: obj-*/dist/zen-<version>.en-US.win64.zip (or .msi)
```

On Windows you must run the build commands inside **MozillaBuild** (`start-shell.bat`) to get the correct PATH for the build toolchain.
